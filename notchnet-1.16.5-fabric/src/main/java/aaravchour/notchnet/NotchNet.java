...
public class NotchNet implements ModInitializer {
	...
	public void onInitialize() {
		...
		System.out.println("[NotchNet 1.16.5] Loaded successfully!");
	}
	...
	private static void registerCommands(CommandDispatcher<ServerCommandSource> dispatcher) {
		dispatcher.register(
			CommandManager.literal("notchnet")
				.executes(ctx -> {
					ctx.getSource().sendFeedback(new LiteralText("§6[NotchNet]§r Use §7/notchnet help§r for commands."), false);
					return 1;
				})
				.then(CommandManager.literal("help")
					.executes(ctx -> {
						ServerCommandSource s = ctx.getSource();
						s.sendFeedback(new LiteralText("§6--- NotchNet Help ---"), false);
						s.sendFeedback(new LiteralText("§7/notchnet <question> §r- Ask the AI a question."), false);
						s.sendFeedback(new LiteralText("§7/notchnet status §r- Check backend connectivity."), false);
						s.sendFeedback(new LiteralText("§7/notchnet config §r- View/Change settings."), false);
						return 1;
					})
				)
					...
					.executes(ctx -> {
						...
						CompletableFuture.runAsync(() -> {
							try {
								...
								s.sendFeedback(new LiteralText("§a✅ Connected to Backend! §7(Status: " + code + ")"), false);
							} catch (Exception e) {
								s.sendFeedback(new LiteralText("§c❌ Connection Failed: §r" + e.getMessage()), false);
							}
						});
						return 1;
					})
				...
				.then(CommandManager.literal("config")
					.executes(ctx -> {
						ServerCommandSource s = ctx.getSource();
						s.sendFeedback(new LiteralText("§6--- NotchNet Config ---"), false);
						s.sendFeedback(new LiteralText("§7apiUrl: §r" + NotchNetConfig.apiUrl), false);
						s.sendFeedback(new LiteralText("§7autoScanMods: §r" + NotchNetConfig.autoScanMods), false);
						return 1;
					})
					.then(CommandManager.literal("apiUrl")
						.then(CommandManager.argument("url", StringArgumentType.string())
							.executes(ctx -> {
								...
								ctx.getSource().sendFeedback(new LiteralText("§a✅ API URL updated to: §r" + NotchNetConfig.apiUrl), false);
								return 1;
							})
						)
					)
						...
						CompletableFuture.runAsync(() -> {
							try {
								...
								for (String line : answer.split("\n")) {
									source.sendFeedback(new LiteralText("§f" + line), false);
								}
							} catch (Exception e) {
								source.sendFeedback(new LiteralText("§c⚠️ Error: §r" + e.getMessage()), false);
							}
						});
						return 1;
					})
